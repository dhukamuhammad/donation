...
export async function GET(req, { params }) {
    try {
        ...
        const [rows] = await db.query(
            "SELECT * FROM donation_fund WHERE id = ?",
            [id]
        );
...
export async function PUT(req, { params }) {
    try {
        ...
        if (file && file.size > 0) {
            ...
            if (old.thumbnail) {
                await fs.unlink(
                    path.join(process.cwd(), "public/uploads", old.thumbnail)
                ).catch(() => { });
            }
        }
        ...
        await db.query(
            ...
            [fields.title, fields.date, fields.total_amount, imagePath, fields.fun_cat, id]
        );
